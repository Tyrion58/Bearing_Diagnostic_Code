...
def get_files(root, test=False):
    ...
    data_root2 = os.path.join(root, datasetname[2])
    path1 = os.path.join(data_root1, normalname[3])  # 0->1797rpm ;1->1772rpm;2->1750rpm;3->1730rpm
    data, lab = data_load(path1, axisname=normalname[3], label=0)  # The label for normal data is 0

    for i in tqdm(range(len(dataname12))):
        path2 = os.path.join(data_root2, dataname12[i])
...
class CWRU_LAP(object):
    ...
    def data_preprare(self, test=False):
        ...
        if test:
            ...
            self.train_pd, val_pd = train_test_split(data_pd, test_size=0.20, random_state=40, stratify=data_pd["label"])
            train_dataset = dataset(list_data=self.train_pd, transform=data_transforms('train', self.normlizetype))
            val_dataset = dataset(list_data=val_pd, transform=data_transforms('val', self.normlizetype))
            train_dataset.R = self.Knn(10)
            return train_dataset, val_dataset

    def Knn(self, k=10):
        # 如果使用Laplace图嵌入，需要用Knn计算得到R
        # 获取数据
        train_data = np.array(self.train_pd['data'].tolist()).reshape(-1, 1024)
        # size == (3800, 1024)
        nbrs = NearestNeighbors(n_neighbors=k, algorithm='ball_tree').fit(train_data)
        ...
        for i in range(number):
            for j in range(k):
                W[i, indexs[i, j]] = math.exp(-distances[i, j] / 2 * self.sita * self.sita)
        # 求度矩阵
        d = np.sum(W, axis=1)
        D = np.diag(d)
        # 得到拉普拉斯矩阵
