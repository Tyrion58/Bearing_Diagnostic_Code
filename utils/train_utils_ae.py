...
class train_utils(object):
    ...
    def setup(self):
        ...
        self.decoder = getattr(fmodel, 'decoder')(in_channel=Dataset.inputchannel, out_channel=Dataset.num_classes)
        ...
        if args.lr_scheduler == 'step':
            steps1 = [int(step) for step in args.steps.split(',')]
            self.lr_scheduler = optim.lr_scheduler.MultiStepLR(self.optimizer, steps1, gamma=args.gamma)
        ...
        if args.opt == 'sgd':
            self.optimizer1 = optim.SGD(
                [{'params': self.encoder.parameters()}, {'params': self.classifier.parameters()}],
            ...
            for phase in ['train', 'val']:
                ...
                for batch_idx, (inputs, labels) in enumerate(self.dataloaders[phase]):
                    ...
                    with torch.set_grad_enabled(phase == 'train'):
                        # 前向传播，forward
                        if args.model_name in ["Sae", "SSDSae"]:
                            z = self.encoder(inputs)
                            recx = self.decoder(z)
                            if args.model_name == "Sae":
                                loss = SAEloss(recx, inputs, z)
                            # 如果需要用到拉普拉斯图嵌入
                            if args.data_name in ["CWRU_LAP"]:
                                loss += args.epsilon * self.datasets['train'].R
                        elif args.model_name in ["Dae", "Ae1d", "Ae2d", "Dae1d", "Dae2d"]:
                            z = self.encoder(inputs)
